...
chat_puts() {
  cur_get
  cat "$DIR/$TIT" | while IFS= read -r line;do
    msg_puts "$line"
  done
}
...
chat_retry() {
  cur_get
  msg=$(cat "$DIR/$TIT" | while read -r line;do
      printf "%s\n" "$s$line"
      s=,
    done)
...
cur_get() {
  ...
  OUT=$(tail -n 1 "$DIR/$CUR")
  [ "$?" -gt 0 ] && err_exit "Error reading chat from current filename"
